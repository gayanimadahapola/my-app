...
class Hero extends Component{
    render() {
        return(
          ...
          <div className="row justify-content-center" >
              ...
              >
                  Like {" "}
              <span class="badge badge-light" >{this.props.hero.likeCount}</span>
              </button>
              </div>        
           </div>          
        </div>
    ...
    showMovies(){
        if (!this.props.hero.movies){
            return;
        }
        if(this.props.hero.movies.length === 0) return <p>No Movies available</p>;
    return this.props.hero.movies.map(movie => <li 
        key={this.props.hero.movies.indexOf(movie)} >{movie}</li>);
    }
    ...
    //create method
    isHero(){
        if(this.props.hero.id < 1000) {
        return "Super Girl";
    } else {
        return "Not an Avenger";
    }
    //below is simple code for above
    return  (this.props.hero.id < 1000) ? "Avenger" : "Not an Avenger";
 }
}
